...
const int mqtt_port = 1883; // Standard MQTT port for microcontrollers

// Topic names matching your dashboard configurations
const char* topic_temp = "digester/sensors/temperature";
const char* topic_humid = "digester/sensors/humidity";
...
// --- Your HTML Dashboard Embedded as a Separate Variable ---
...
<head>
    ...
    <style>
        ...
        .humid-card { border-top: 4px solid var(--accent-humid); }
...
<body>
            ...
            <section class="telemetry-column">
                <div class="metrics-grid">
                    ...
                    <div class="card humid-card">
                        ...
                        <div class="card-value"><span id="humid">--</span>%</div>
                    </div>
    ...
    <script>
        ...
        // Change these values to match your specific setup (e.g., HiveMQ Cloud, local Raspberry Pi)
        const MQTT_BROKER = "broker.hivemq.com"; 
        const MQTT_PORT = 8000; // MUST be the WebSocket port of the broker, NOT 1883
        ...
        const TOPIC_HUMID = "digester/sensors/humidity";
        const TOPIC_METHANE = "digester/sensors/methane";
        const TOPIC_COMMANDS = "digester/control/commands"; // Topic where commands will be sent
        ...
        function initMQTT() {
            ...
            const options = {
                ...
                useSSL: false // Change to true if using a secured wss:// broker URL connection
            };
        ...
        function onMessageArrived(message) {
            ...
            if (topic === TOPIC_TEMP) {
                ...
                document.getElementById('humid').innerText = floatVal.toFixed(1);
            ...
            // Push to line chart whenever a main dataset variable arrives
            if (dataUpdated && sensorChart) {
                // If it's a new time bracket, push a new label block
                if (sensorChart.data.labels.length === 0 || sensorChart.data.labels[sensorChart.data.labels.length - 1] !== timestamp) {
                    ...
                    // Update current structural coordinates frame placeholder data in place
        ...
        function sendCommand() {
            ...
            // Publish command out to the network broker
...
void reconnectMQTT() {
    // Loop until we are reconnected to the HiveMQ broker
...
void setup() {
    ...
    // Set up MQTT server link parameters
    mqttClient.setServer(mqtt_server, mqtt_port);

    // Route for root / web page to deliver your embedded HTML
...
void loop() {
    ...
    mqttClient.loop(); // Keeps the MQTT client processing loops alive

    if (sensorTimer.isReady()) {
        // 1. Fetch physical DHT11 environmental values
        DHTData dhtData = readDHT();
        float currentHumid = dht.readHumidity(); // Adding missing humidity tracking channel
        
        // 2. Fetch physical DS18B20 target core probe readings
        float dsTemp = readDS18B20();

        // Standard console check debugging print lines
        Serial.print("Ambient: "); Serial.print(dhtData.temperature);
        Serial.print("C | Core: "); Serial.print(dsTemp);
        Serial.print("C | Humid: "); Serial.print(currentHumid); Serial.println("%");

        // Convert data parameters to character strings for MQTT delivery
        char tempString[8];
        char humidString[8];
        
        dtostrf(dsTemp, 1, 2, tempString);
        dtostrf(currentHumid, 1, 2, humidString);

        // 3. Publish directly to the shared HiveMQ stream
        // Your dashboard will catch these payloads automatically!
        mqttClient.publish(topic_temp, tempString);
        mqttClient.publish(topic_humid, humidString);
    }
}